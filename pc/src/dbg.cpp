    ...
    class dbg_ostream : public std::ostream
    {
        public:
            ...
            dbg_ostream(const dbg_ostream &rhs)
                : std::basic_ios<char>(&dbg_buf) , std::basic_ostream<char>(&dbg_buf), streams(rhs.streams),
                  dbg_buf(streams){}
#else
...
namespace
{
    ...
    void print_pos(std::ostream &out, const dbg::source_pos &where)
    {
        if (where.file)
        {
			std::string file = where.file;
           if (where.func)
           {
               out << "function: " << where.func << ", ";
           }
           out << "line: " << where.line << ", file: " << file;
        }
    }
